...
/**
 * This is the base class for any KinesisConnector. It is configured by a
 * constructor that takes in as parameters implementations of the IBuffer,
 * ITransformer, and IEmitter dependencies defined in a
 * IKinesisConnectorPipeline. It is typed to match the class that records are
 * transformed into for filtering and manipulation. This class is produced by a
 * KinesisConnectorRecordProcessorFactory.
 * <p>
 * When a Worker calls processRecords() on this class, the pipeline is used in
 * the following way:
 * <ol>
 * <li>Records are transformed into the corresponding data model (parameter type
 * T) via the ITransformer.</li>
 * <li>Transformed records are passed to the IBuffer.consumeRecord() method,
 * which may optionally filter based on the IFilter in the pipeline.</li>
 * <li>When the buffer is full (IBuffer.shouldFlush() returns true), records are
 * transformed with the ITransformer to the output type (parameter type U) and a
 * call is made to IEmitter.emit(). IEmitter.emit() returning an empty list is
 * considered a success, so the record processor will checkpoint and emit will
 * not be retried. Non-empty return values will result in additional calls to
 * emit with failed records as the unprocessed list until the retry limit is
 * reached. Upon exceeding the retry limit or an exception being thrown, the
 * IEmitter.fail() method will be called with the unprocessed records.</li>
 * <li>When the shutdown() method of this class is invoked, a call is made to
 * the IEmitter.shutdown() method which should close any existing client
 * connections.</li>
 * </ol>
 *
 */
public class KinesisConnectorRecordProcessor<T, U> implements IRecordProcessor {
    ...
    public KinesisConnectorRecordProcessor(IBuffer<T> buffer, IFilter<T> filter, IEmitter<U> emitter,
            ITransformerBase<T, U> transformer, KinesisConnectorConfiguration configuration) {
    ...
    public void processRecords(List<Record> records, IRecordProcessorCheckpointer checkpointer) {
        // Note: This method will be called even for empty record lists. This is needed
        // for checking the buffer time
        // threshold.
        if (isShutdown) {
